...
type pageCommon struct {
	...
	toDex                   decredmaterial.IconButton
...
func (w *Wallet) addPages(decredIcons map[string]image.Image) {
	ic := pageIcons{
	...
	common := pageCommon{
		...
		theme:                   w.theme,
		icons:                   ic,
		returnPage:              &w.previous,
		page:                    &w.current,
		walletTabs:              w.walletTabs,
		accountTabs:             w.accountTabs,
		...
		minimizeNavDrawerButton: w.theme.PlainIconButton(new(widget.Clickable), ic.navigationArrowBack),
		toDex:                   w.theme.PlainIconButton(new(widget.Clickable), ic.cached),
		maximizeNavDrawerButton: w.theme.PlainIconButton(new(widget.Clickable), ic.navigationArrowForward),
		selectedUTXO:            make(map[int]map[int32]map[string]*wallet.UnspentOutput),
		modal:                   w.theme.Modal(),
		modalReceiver:           w.modal,
		modalLoad:               &modalLoad{},
		subPageBackButton:       w.theme.PlainIconButton(new(widget.Clickable), ic.navigationArrowBack),
		subPageInfoButton:       w.theme.PlainIconButton(new(widget.Clickable), ic.actionInfo),
		changePage:              w.changePage,
		setReturnPage:           w.setReturnPage,
		refreshWindow:           w.refresh,

		switchView: w.switchView,
	}
	...
	common.testButton = w.theme.Button(new(widget.Clickable), "test button")
	...
	common.modalTemplate = w.LoadModalTemplates()

	w.pages = make(map[string]layout.Widget)
	w.pages[PageWallet] = w.WalletPage(common)
	w.pages[PageOverview] = w.OverviewPage(common)
	w.pages[PageTransactions] = w.TransactionsPage(common)
	w.pages[PageMore] = w.MorePage(common)
	w.pages[PageCreateRestore] = w.CreateRestorePage(common)
	w.pages[PageReceive] = w.ReceivePage(common)
	w.pages[PageSend] = w.SendPage(common)
	w.pages[PageTransactionDetails] = w.TransactionDetailsPage(common)
	w.pages[PageSignMessage] = w.SignMessagePage(common)
	w.pages[PageVerifyMessage] = w.VerifyMessagePage(common)
	w.pages[PageSeedBackup] = w.BackupPage(common)
	w.pages[PageSettings] = w.SettingsPage(common)
	w.pages[PageWalletSettings] = w.WalletSettingsPage(common)
	w.pages[PageSecurityTools] = w.SecurityToolsPage(common)
	w.pages[PageProposals] = w.ProposalsPage(common)
	w.pages[PageProposalDetails] = w.ProposalDetailsPage(common)
	w.pages[PageDebug] = w.DebugPage(common)
	w.pages[PageLog] = w.LogPage(common)
	w.pages[PageAbout] = w.AboutPage(common)
	w.pages[PageHelp] = w.HelpPage(common)
	w.pages[PageUTXO] = w.UTXOPage(common)
	w.pages[PageAccountDetails] = w.AcctDetailsPage(common)
	w.pages[PagePrivacy] = w.PrivacyPage(common)
	w.pages[PageTickets] = w.TicketPage(common)
	w.pages[ValidateAddress] = w.ValidateAddressPage(common)
}
