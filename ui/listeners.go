...
func (mp *mainPage) desktopNotifier() {
	go func() {
		for {
			var notifier interface{}

			select {
			case notifier = <-mp.notificationsUpdate:
			}
			...
			case wallet.Proposal:
				notification = fmt.Sprintf("Net sure what i stored here : %T", t)
				fmt.Printf("Net sure what i stored here : %T", t)
				switch {
