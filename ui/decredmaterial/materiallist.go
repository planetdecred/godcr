...
func fromListPosition(lp LayoutPosition, elements int, majorAxisSize int) (start, end float32) {
...
// theme and state.
func GetScrollbar(th *Theme, state *Scrollbar) ScrollbarStyle {
	lightFg := th.Base.Palette.Fg
	lightFg.A = 150
	darkFg := lightFg
	darkFg.A = 200
...
func (s ScrollbarStyle) Layout(gtx layout.Context, axis layout.Axis, viewportStart, viewportEnd float32) layout.Dimensions {
	...
	s.Scrollbar.Layout(gtx, axis, viewportStart)
...
type ListStyle struct {
	state *ListWithScroll
	ScrollbarStyle
	AnchorStrategy
}

// List constructs a ListStyle using the provided theme and state.
func List(th *Theme, state *ListWithScroll) ListStyle {
	return ListStyle{
		state:          state,
		ScrollbarStyle: GetScrollbar(th, &state.Scrollbar),
	}
}

// Layout the list and its scrollbar.
func (l ListStyle) Layout(gtx layout.Context, length int, w ListElement) layout.Dimensions {
	...
	listDims := l.state.LayoutList.Layout(gtx, length, w)
	...
	start, end := fromListPosition(l.state.LayoutPosition, length, majorAxisSize)
	...
	if delta := l.state.ScrollDistance(); delta != 0 {
		...
		// with the scrollbar.
		deltaPx := int(math.Round(float64(float32(l.state.LayoutPosition.Length) * delta)))
		l.state.LayoutList.LayoutPosition.Offset += deltaPx
		...
		l.state.LayoutList.LayoutPosition.BeforeEnd = true
	}
