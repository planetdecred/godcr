...
type WalletSettingsPage struct {
	*app.App
	PopFragment func() // TODO: Implement / improve.
...
func NewWalletSettingsPage(app *app.App, wal *dcrlibwallet.Wallet) *WalletSettingsPage {
	pg := &WalletSettingsPage{
		App:          app,
		wallet:       wal,
		changePass:   app.Theme.NewClickable(false),
		rescan:       app.Theme.NewClickable(false),
		deleteWallet: app.Theme.NewClickable(false),

		chevronRightIcon: decredmaterial.NewIcon(app.Theme.Icons.ChevronRight),
	}
...
func (pg *WalletSettingsPage) HandleUserInteractions() {
	for pg.changePass.Clicked() {
		modal.NewPasswordModal(pg.Theme, nil).
				...
				go func() {
					...
					modal.NewCreatePasswordModal(nil).
						...
						PasswordCreated(func(walletName, newPassword string, m *modal.CreatePasswordModal) bool {
							go func() {
								err := pg.MultiWallet().ChangePrivatePassphraseForWallet(pg.wallet.ID, []byte(password),
		...
		go func() {
			info := modal.NewInfoModal(pg.App).
				...
				PositiveButton(values.String(values.StrRescan), func(isChecked bool) {
					err := pg.MultiWallet().RescanBlocks(pg.wallet.ID)
					if err != nil {
	...
	for pg.deleteWallet.Clicked() {
		...
		modal.NewInfoModal(pg.App).
			...
			PositiveButton(values.String(values.StrRemove), func(isChecked bool) {
				walletDeleted := func() {
					if pg.MultiWallet().LoadedWalletsCount() > 0 {
				...
				if pg.wallet.IsWatchingOnlyWallet() {
					...
					err := pg.MultiWallet().DeleteWallet(pg.wallet.ID, nil)
					if err != nil {
						pg.Toast.NotifyError(err.Error())
					} else {
						walletDeleted()
				...
				modal.NewPasswordModal(pg.Theme, pg.App).
					...
					PositiveButton(values.String(values.StrConfirm), func(password string, pm *modal.PasswordModal) bool {
						go func() {
							err := pg.MultiWallet().DeleteWallet(pg.wallet.ID, []byte(password))
							if err != nil {
								pm.SetError(err.Error())
								pm.SetLoading(false)
								return
