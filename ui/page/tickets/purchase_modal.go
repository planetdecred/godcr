...
func (tp *ticketPurchaseModal) OnResume() {
	...
	if err != nil {
		tp.Toast.Notify(err.Error(), false)
	}
...
func (tp *ticketPurchaseModal) purchaseTickets(password []byte) {
	tp.Dismiss()
	tp.Toast.Notify(fmt.Sprintf("attempting to purchase %v ticket(s)", tp.ticketCount()), true)
	...
	go func() {
		...
		tp.Toast.Notify(fmt.Sprintf("%v ticket(s) purchased successfully", tp.ticketCount()), true)
	}()
}
