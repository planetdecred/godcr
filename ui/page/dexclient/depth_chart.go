...
func NewDepthChart(buys, sells []*core.MiniOrder, theme *decredmaterial.Theme) *DepthChart {
	increase := theme.IconButton(decredmaterial.MustIcon(widget.NewIcon(icons.ContentAdd)))
	decrease := theme.IconButton(decredmaterial.MustIcon(widget.NewIcon(icons.ContentRemove)))
