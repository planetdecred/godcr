...
func (cfReg *confirmRegistration) confirm(feeAssetName string, password string, cert []byte) {
	modal.NewInfoModal(cfReg.Load).
		Title(strConfirmReg).
		Body(confirmRegisterModalDesc(cfReg.ex, feeAssetName)).
		SetCancelable(false).
		...
		PositiveButton(strRegister, func() {
			go func() {
				...
				*cfReg.isSending = true
				_, err := cfReg.Dexc().RegisterWithDEXServer(cfReg.ex.Host,
					cert,
					int64(cfReg.ex.Fee.Amt),
					int32(cfReg.ex.Fee.ID),
					[]byte(password))
				if err != nil {
